...
function set_checked_state(val, item_id, subcat_idx)
{
    $(`[data-item_id='${item_id}'][data-subcat_idx=${subcat_idx}]`).data('checked', val);
}

function gen_subcat_general(item_id, subcat_id, param, price, amount, fav)
{
    subcat_html = `
    <tr class='${fav?'fav':''}' data-subcat_id='${subcat_id}' data-item_id='${item_id}'>
        <td>
...
function gen_admin_table_rows(item)
{
    return `
        <tr data-item_id='${item.id}' data-role='main_item_data'>
            ...
            <td colspan=0>
                <table border="0" data-subcat_table='${item.id}' style='width: 100%'>
                    <thead>
                        <tr>
...
function gen_user_table_rows(item)
{
    ...
    <table class='table table-bordered'>
        ...
        <tbody>
            ...
            <tr><td class='flex'>${gen_image_bulk(item.photo_paths, item.id)}</td></tr>
            <tr>
                <td>
...
function add_table_contents(data)
{
    for (let [category, items] of data)
    {
        ...
        {
            if ( the_searcher.sorts_by_category() )
            {
                // append to category
                if ( $(`#table-container div[data-category='${category}']`).length == 0 )
                {
                    console.log('no div found add one', gen_cat_div(category) )
                    $(`#table-container`).append( gen_cat_div(category) )
                }
                $(`#table-container div[data-category='${category}'] `).append(gen_table_rows(item))
            }
            else
            {
                // append to end
                $(`#table-container`).append(gen_table_rows(item));
            }
